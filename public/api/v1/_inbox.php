 ...
 * PATCH  /inbox/{recipient_id}     — update status (read/archived/deleted/done/seen/in_progress/cancelled/blocked/needs_human)
 */
...
switch ($method) {
    case 'GET':
        if ($id_param) {
            ...
            $valid_statuses = ['unread', 'read', 'archived', 'done', 'seen', 'in_progress', 'cancelled', 'blocked', 'needs_human', 'all'];
    ...
    case 'PATCH':
        ...
        if (!in_array($new_status, ['read', 'archived', 'deleted', 'done', 'seen', 'in_progress', 'cancelled', 'blocked', 'needs_human'])) {
            http_response_code(400);
            echo json_encode(['error' => 'status must be one of: read, archived, deleted, done, seen, in_progress, cancelled, blocked, needs_human']);
            break;
        }
